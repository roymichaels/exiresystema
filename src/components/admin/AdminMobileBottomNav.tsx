...
export function AdminMobileBottomNav({ activeTab, onTabChange }: Props) {
  ...
  return (
    ...
    >
      <ul className="mx-auto flex items-stretch justify-around h-full px-1 max-w-md">
        {primary.map((tab) => {
          const Icon = tab.icon;
          const isActive =
            activeTab === tab.id ||
          ...
          return (
            <li key={tab.id} className="flex-1">
              <button
                ...
                className={cn(
                  'w-full flex flex-col items-center justify-center gap-0.5 transition-all h-full min-h-[48px] relative rounded-xl',
              ...
              >
                {isActive && (
                  <span className="absolute -top-[5px] left-1/2 -translate-x-1/2 h-1 w-7 rounded-full bg-primary" />
                )}
                <Icon
                  className={cn('w-[18px] h-[18px]', isActive && 'text-primary')}
                  strokeWidth={isActive ? 2.2 : 1.6}
                />
                <span className={cn('text-[10px] leading-none tracking-tight', isActive ? 'font-semibold' : 'font-medium')}>
