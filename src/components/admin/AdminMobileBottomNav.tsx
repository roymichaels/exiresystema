          ...
          return (
            <li key={tab.id} className="flex-1">
              <button
                ...
                className={cn(
                  'w-full flex flex-col items-center gap-0 py-1 transition-colors',
                  isActive ? 'text-foreground' : 'text-muted-foreground/60',
                )}
                aria-current={isActive ? 'page' : undefined}
              >
                <Icon
                  className={cn('w-[15px] h-[15px]', isActive && 'text-primary')}
                  strokeWidth={isActive ? 2 : 1.5}
                />
                <span className={cn('text-[9px] leading-tight mt-px', isActive ? 'font-semibold' : 'font-medium')}>
