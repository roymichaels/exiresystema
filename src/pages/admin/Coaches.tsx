...
export default function AdminCoaches() {
  ...
  const updateCoach = useMutation({
    ...
    onSuccess: () => {
      ...
      toast.success(language === 'he' ? 'המאמן עודכן בהצלחה' : language === 'es' ? 'Coach actualizado exitosamente' : 'Coach updated successfully');
      setEditingCoach(null);
    },
    onError: () => {
      toast.error(language === 'he' ? 'שגיאה בעדכון' : language === 'es' ? 'Error al actualizar' : 'Update failed');
    },
  });
  ...
  return (
    <div className="space-y-6" dir={isHe ? 'rtl' : 'ltr'}>
      <Tabs defaultValue="coaches" className="space-y-4">
        <TabsList>
          <TabsTrigger value="coaches" className="gap-1">
            ...
            {language === 'he' ? 'מאמנים' : language === 'es' ? 'Entrenadores' : 'Coaches'}
          </TabsTrigger>
          <TabsTrigger value="coupons" className="gap-1">
            <Ticket className="h-4 w-4" />
            {language === 'he' ? 'קופונים' : language === 'es' ? 'Cupones' : 'Coupons'}
          </TabsTrigger>
        </TabsList>
      ...
      <div className="flex items-center justify-between">
        <h2 className="text-2xl font-bold">{language === 'he' ? 'ניהול מאמנים' : language === 'es' ? 'Gestión de entrenadores' : 'Coaches Management'}</h2>
        <Badge variant="secondary" className="text-sm">
          {coaches?.length || 0} {language === 'he' ? 'מאמנים' : language === 'es' ? 'entrenadores' : 'coaches'}
        </Badge>
      </div>
      ...
      <div className="flex flex-wrap gap-3">
        <div className="relative flex-1 min-w-[200px]">
          ...
          <Input
            placeholder={language === 'he' ? 'חיפוש מאמן...' : language === 'es' ? 'Buscar entrenador...' : 'Search coach...'}
            value={search}
        ...
        <Select value={statusFilter} onValueChange={setStatusFilter}>
          ...
          <SelectContent>
            <SelectItem value="all">{language === 'he' ? 'כל הסטטוסים' : language === 'es' ? 'Todos los estados' : 'All statuses'}</SelectItem>
            <SelectItem value="active">{language === 'he' ? 'פעיל' : language === 'es' ? 'Activo' : 'Active'}</SelectItem>
            <SelectItem value="pending">{language === 'he' ? 'ממתין' : language === 'es' ? 'Pendiente' : 'Pending'}</SelectItem>
            <SelectItem value="inactive">{language === 'he' ? 'לא פעיל' : language === 'es' ? 'Inactivo' : 'Inactive'}</SelectItem>
          </SelectContent>
        </Select>
      </div>
      ...
      {/* Coaches Grid */}
      {isLoading ? (
        <div className="text-center py-12 text-muted-foreground">{language === 'he' ? 'טוען...' : language === 'es' ? 'Cargando...' : 'Loading...'}</div>
      ) : !filtered?.length ? (
        <div className="text-center py-12 text-muted-foreground">{language === 'he' ? 'לא נמצאו מאמנים' : language === 'es' ? 'No se encontraron entrenadores' : 'No coaches found'}</div>
      ) : (
              ...
              <CardContent className="space-y-3">
                ...
                <div className="flex gap-4 text-sm">
                  ...
                  <span className="flex items-center gap-1 text-muted-foreground">
                    ...
                    {coach.clients_count || 0} {language === 'he' ? 'לקוחות' : language === 'es' ? 'clientes' : 'clients'}
                  </span>
                </div>
                ...
                <div className="flex items-center gap-4">
                  <label className="flex items-center gap-2 text-sm cursor-pointer">
                    ...
                    />
                    <Star className="h-3.5 w-3.5" />
                    {language === 'he' ? 'מובחר' : language === 'es' ? 'Destacado' : 'Featured'}
                  </label>
                  <label className="flex items-center gap-2 text-sm cursor-pointer">
                    <Switch
                    ...
                    {language === 'he' ? 'מאומת' : language === 'es' ? 'Verificado' : 'Verified'}
                  </label>
                </div>
                ...
                <div className="flex gap-2 pt-1">
                  ...
                  <Button size="sm" variant="ghost" asChild>
                    <a href={`/coaches/${coach.slug}`} target="_blank" rel="noopener">
                      ...
                      {language === 'he' ? 'צפייה' : language === 'es' ? 'Ver' : 'View'}
                    </a>
                  </Button>
                </div>
              </CardContent>
      ...
      <Dialog open={!!editingCoach} onOpenChange={open => !open && setEditingCoach(null)}>
        <DialogContent className="max-w-lg max-h-[85vh] overflow-y-auto" dir={isHe ? 'rtl' : 'ltr'}>
          <DialogHeader>
            <DialogTitle>{language === 'he' ? 'עריכת מאמן' : language === 'es' ? 'Editar entrenador' : 'Edit Coach'}</DialogTitle>
          </DialogHeader>
          {editingCoach && (
            <div className="space-y-4">
              <div className="grid grid-cols-2 gap-3">
                <div className="space-y-1.5">
                  <Label>{language === 'he' ? 'שם (עברית)' : language === 'es' ? 'Nombre (Hebreo)' : 'Name (Hebrew)'}</Label>
                  <Input value={editingCoach.display_name} onChange={e => setEditingCoach({ ...editingCoach, display_name: e.target.value })} />
                </div>
                <div className="space-y-1.5">
                  <Label>{language === 'he' ? 'שם (אנגלית)' : language === 'es' ? 'Nombre (Inglés)' : 'Name (English)'}</Label>
              ...
              <div className="grid grid-cols-2 gap-3">
                <div className="space-y-1.5">
                  <Label>{language === 'he' ? 'תפקיד (עברית)' : language === 'es' ? 'Título (Hebreo)' : 'Title (Hebrew)'}</Label>
                  <Input value={editingCoach.title} onChange={e => setEditingCoach({ ...editingCoach, title: e.target.value })} />
                </div>
                <div className="space-y-1.5">
                  <Label>{language === 'he' ? 'תפקיד (אנגלית)' : language === 'es' ? 'Título (Inglés)' : 'Title (English)'}</Label>
                  <Input value={editingCoach.title_en || ''} onChange={e => setEditingCoach({ ...editingCoach, title_en: e.target.value })} />
                </div>
              </div>

              <div className="space-y-1.5">
                <Label>{language === 'he' ? 'ביוגרפיה (עברית)' : language === 'es' ? 'Biografía (Hebreo)' : 'Bio (Hebrew)'}</Label>
                <Textarea rows={3} value={editingCoach.bio || ''} onChange={e => setEditingCoach({ ...editingCoach, bio: e.target.value })} />
              </div>

              <div className="space-y-1.5">
                <Label>{language === 'he' ? 'ביוגרפיה (אנגלית)' : language === 'es' ? 'Biografía (Inglés)' : 'Bio (English)'}</Label>
              ...
              <div className="space-y-1.5">
                <Label>{language === 'he' ? 'עמלה (%)' : language === 'es' ? 'Comisión (%)' : 'Commission (%)'}</Label>
